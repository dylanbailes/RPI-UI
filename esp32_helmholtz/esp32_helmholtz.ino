...
const int CS_PIN = 39; // Added for electrode current sensing
...
// Electrode sensing constants (from sketch_jun7a)
const float R_SENSE      = 2490.0f;
const float IPROPI_RATIO = 1000.0f;
const int   CS_SAMPLES_FAST = 16; // Optimized for 500us loop time (was 256)
...
// --- Fast CS Pin Voltage Read (Optimized for 2000Hz loop) ---
float readCsVoltage() {
  long sum = 0;
  for (int i = 0; i < CS_SAMPLES_FAST; i++) {
    sum += esp_adc_cal_raw_to_voltage(analogRead(CS_PIN), &adc_chars);
  }
  float mv = ((sum / (float)CS_SAMPLES_FAST) - adcZeroOffset_mV) * ADC_CAL;
  if (mv < 0.0f) mv = 0.0f;
  return mv / 1000.0f; // Return in Volts
}

// --- Magnetic Calibration Helpers ---
...
void calibrateAdc() {
  ...
  Serial.println("Enter multimeter reading in mV at each step, then press Enter");
  float rawPoints[11];
  float truePoints[11];
  ...
  for (int i = 0; i <= 10; i++) {
    float pct = i * 10.0f;
    portENTER_CRITICAL(&paramMux);
    elecWaveform    = (pct == 0) ? WAVE_OFF : WAVE_STEP;
    elecAmpPercent  = pct;
    elecWaveStartUs = micros();
    portEXIT_CRITICAL(&paramMux);
    delay(300);
    ...
    float espMv = (sum / 64.0f) - adcZeroOffset_mV;
    if (espMv < 0.0f) espMv = 0.0f;
    rawPoints[i] = espMv;
    
    Serial.print("Step "); Serial.print((int)pct);
    Serial.print("% | ESP32="); Serial.print(espMv, 1);
    Serial.println("mV | Enter multimeter mV:");
    ...
    while (true) {
      if (Serial.available()) {
        ...
        if (c == '\n' || c == '\r') {
          if (inputLen > 0) {
            inputBuf[inputLen] = '\0';
            truePoints[i] = atof(inputBuf);
            Serial.print("  -> Recorded: ");
            Serial.print(truePoints[i], 1); Serial.println("mV");
            inputLen = 0;
            break;
          }
        } else if (inputLen < 15) {
  ...
  elecWaveform = WAVE_OFF;
  portEXIT_CRITICAL(&paramMux);
  ledcWrite(ELEC_IN1_PIN, 0);
  ledcWrite(ELEC_IN2_PIN, 0);
  ...
  for (int i = 0; i <= 10; i++) {
    float pct = i * 10.0f;
    if (rawPoints[i] > 5.0f) {
      float scale = truePoints[i] / rawPoints[i];
      scaleSum += scale;
      validPoints++;
      Serial.print("  "); Serial.print((int)pct);
      Serial.print("%: ESP32="); Serial.print(rawPoints[i], 1);
      Serial.print("mV  True="); Serial.print(truePoints[i], 1);
      Serial.print("mV  scale="); Serial.println(scale, 4);
    } else {
      Serial.print("  "); Serial.print((int)pct);
      Serial.println("%: skipped (near zero)");
    }
  }
  
  if (validPoints > 0) {
    ADC_CAL = scaleSum / validPoints;
    Serial.print("\nADC_CAL set to: "); Serial.println(ADC_CAL, 4);
    Serial.println("Calibration complete, readings now corrected");
  } else {
    Serial.println("ERR: no valid points, calibration failed");
  }
}
...
void setup() {
  ...
  analogSetPinAttenuation(CS_PIN, ADC_11db);
  
  esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTEN_DB_11, ADC_WIDTH_BIT_12, 1100, &adc_chars);
  
  // Measure CS pin zero offset with output off
  float zeroSum = 0;
  for (int i = 0; i < 64; i++) {
    zeroSum += esp_adc_cal_raw_to_voltage(analogRead(CS_PIN), &adc_chars);
    delayMicroseconds(100);
  }
  adcZeroOffset_mV = zeroSum / 64.0f;
  ...
  Serial.println("  a                        - run ADC calibration for CS pin");
  Serial.println("  v                        - read CS voltage and current manually");
...
void handleCommand(String line) {
  ...
  if (target == 'v' || target == 'V') {
    float v_cs = readCsVoltage();
    float i_motor = (v_cs / R_SENSE) * IPROPI_RATIO;
    Serial.print("V_CS="); Serial.print(v_cs * 1000.0f, 2); Serial.print("mV");
    Serial.print("  I=");  Serial.print(i_motor * 1000.0f, 2); Serial.println("mA");
    return;
  }
...
void loop() {
  ...
  if (nowUs - prevHeSampleUs >= HE_SAMPLE_INTERVAL_US) {
    ...
    if (!isCalibrating) {
      ...
      float v_cs = readCsVoltage(); // Fast read optimized for 2000Hz loop
      
      // Ensure we have enough buffer space to avoid partial writes
      if (Serial.availableForWrite() >= 32) {
        Serial.print(he1Inst, 2);
        Serial.print(' ');
        Serial.print(he2Inst, 2);
        Serial.print(' ');
        Serial.println(v_cs, 3); // 3rd item: Voltage on CS pin in Volts
      }
    }
  }
