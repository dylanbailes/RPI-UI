/* ============================================================================
 * connection.jsx — Screen 01: assign serial devices to wells
 * ========================================================================== */
function ConnectionScreen({ onConnect, onExit }) {
  const [ports, setPorts] = React.useState(() => window.MCCB.enumeratePorts());
  // default: 3 wells mapped, well 4 left unassigned to exercise the empty state
  const [sel, setSel] = React.useState({ 1: '/dev/ttyUSB0', 2: '/dev/ttyUSB1', 3: '/dev/ttyUSB2', 4: '' });
  const [spin, setSpin] = React.useState(false);

  function refresh() {
    setSpin(true);
    setTimeout(() => { setPorts(window.MCCB.enumeratePorts()); setSpin(false); }, 620);
  }
    ...
    <div className="tab-page enter" style={{ background: '#fff' }}>
      <div className="grow scroll-y" style={{ padding: '34px 40px' }}>
            ...
            {[1, 2, 3, 4].map((n) => {
              ...
              return (
                <div key={n} className="col" style={{ gap: 8 }}>
                  ...
                  <select className="select" value={sel[n]} onChange={(e) => setWell(n, e.target.value)}
                    style={isDupe ? { borderColor: 'var(--accent)' } : null}>
        ...
        <p style={{ color: 'var(--dim)', fontSize: 13, lineHeight: 1.5, marginTop: 18, maxWidth: 620 }}>
          ...
          Wells left as <strong>(NONE)</strong> stay idle and show an unassigned state. {dupes.length > 0 &&
          <span style={{ color: 'var(--accent)', fontWeight: 700 }}> One port is mapped to multiple wells — resolve before continuing.</span>}
        </p>
      </div>
